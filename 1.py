...
def caesar(input, output, offset, chipher):
    fin = open(input, "r")
    fout = open(output, "w")
    for c in fin.read():
        if chipher:
...
def main():
    # files we will work on: a.txt and alice29.txt
    ...
    num_for_shift = 4
    caesar("CD_TestFiles/a.txt", "fileA_CaesarEncipheredOutput.txt", num_for_shift, True)
    caesar("CD_TestFiles/alice29.txt", "fileB_CaesarEncipheredOutput.txt", num_for_shift, True)
    ...
    caesar("fileA_CaesarEncipheredOutput.txt", "fileA_CaesarDecipheredOutput.txt", num_for_shift, False)
    caesar("fileB_CaesarEncipheredOutput.txt", "fileB_CaesarDecipheredOutput.txt", num_for_shift, False)
